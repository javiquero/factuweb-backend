...
let modelsController = {
	...
	// 		if (!fs.existsSync(process.cwd() + "/db.zip")) return reject(new Error("db.zip not exists or is inaccesible.") );

	// 		let zip = new AdmZip(process.cwd() + "/db.zip");
	// 		zip.extractAllToAsync(/*target path*/process.cwd() + "/", /*overwrite*/true, (err) => {
	...
	// 							db.close(async () => {
	// 								await fs.unlinkSync(process.cwd() + "/db.sqlite");
	// 								return resolve();
	// 							})
	// 						} catch (error) {
	...
	async insertOrReplace(req, res) {
		let data = req.allParams();
		// let data = req.param('data', { model: "F_TAR", data: { CODTAR: 4, DESTAR: 'Testo' } });
